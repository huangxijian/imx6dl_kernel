...
/*chiweiwei*/
#define sabresd_FEC_RST      42 /*fr*/
//#define sabresd_FEC_RST      20 /*jh*/
//#define sabresd_FEC_RST      204 /*kl*/
/*3G*/
#define MODERN_DIS       	84 
#define MODERN_RST        85
#define MODERN_WAK        86
#define sabresd_3G_EN            101
...
static void __init imx6q_mini_pcie_init(void)
{
	...
	if (gpio_is_valid(power_on_gpio)) {
		...
		/* reset miniPCIe */
	gpio_request(MODERN_RST, "pcie_reset_b");
	gpio_direction_output(MODERN_RST, 0);
	...
	mdelay(1);
	gpio_set_value(MODERN_RST, 1);
//	gpio_free(MODERN_RST);
	gpio_request(MODERN_DIS , "pcie_dis_b");
	gpio_direction_output(MODERN_DIS , 0);
	/* The PCI Express Mini CEM specification states that PREST# is
	deasserted minimum 1ms after 3.3vVaux has been applied and stable*/
	mdelay(1);
	gpio_set_value(MODERN_DIS , 1);
	gpio_request(MODERN_WAK , "pcie_wakup");
	gpio_direction_output(MODERN_WAK, 0);
	/* The PCI Express Mini CEM specification states that PREST# is
	deasserted minimum 1ms after 3.3vVaux has been applied and stable*/
	mdelay(1);
	gpio_set_value(MODERN_WAK, 1);
}
...
static inline void imx6q_enet_init(void)
{
	...
	imx6q_enet_plt_init();
		/*chiweiwei*/
#if 0
	gpio_request(sabresd_FEC_RST, "fec-rst");
	gpio_direction_output(sabresd_FEC_RST, 0);
	msleep(10);
	gpio_direction_output(sabresd_FEC_RST, 1); 
#endif
}
